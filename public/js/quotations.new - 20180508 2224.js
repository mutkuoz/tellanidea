...
quotation.setCustomer=function(customer){
    ...
    this.existingLendingRevenues=customer.existingLendingRevenues;
    this.existingLendingVolume=customer.existingLendingVolume;
    ...
    if (customer.existingLendingVolume[this.currentYearMonthIndex]>100000000)
        $('#volumeRangeSliderRange').attr({min:0,max:250000000,step:1000000});
    else
        if (customer.existingLendingVolume[this.currentYearMonthIndex]>10000000)
            $('#volumeRangeSliderRange').attr({min:0,max:25000000,step:100000});
            else
                if (customer.existingLendingVolume[this.currentYearMonthIndex]>1000000)
                    $('#volumeRangeSliderRange').attr({min:0,max:2500000,step:10000});
                else
                    $('#volumeRangeSliderRange').attr({min:0,max:250000,step:1000});

    if (customer.existingSideBusinessRevenues[this.currentYearMonthIndex]>1000000)
        $('#sideBusinessRangeSliderRange').attr({min:0,max:25000000,step:10000});
    else
        if (customer.existingSideBusinessRevenues[this.currentYearMonthIndex]>100000)
            $('#sideBusinessRangeSliderRange').attr({min:0,max:2500000,step:1000});
        else
            if (customer.existingSideBusinessRevenues[this.currentYearMonthIndex]>10000)
                $('#sideBusinessRangeSliderRange').attr({min:0,max:250000,step:100});
...
quotation.targetGrowthStepPerMonth=0.0004;
...
var chartRoRWA = Highcharts.chart('container-rorwa', Highcharts.merge(gaugeOptions, {
    yAxis: {
        stops:[
            [0,'#bf000c'],
            [quotation.minimumAcceptableRoRWA/0.05,'#bf000c'],
            [quotation.minimumAcceptableRoRWA/0.05+0.0000001,'#dedf00'],
            [quotation.seriesRORWA_target[quotation.targetTimeFrameIndex]/0.0500000,'#dedf00'],
            [quotation.seriesRORWA_target[quotation.targetTimeFrameIndex]/0.0500000+0.0000000001,'#00bf3d'],
            [1, '#00bf3d']
        ],
        min: 0,
        max: 5/100,
...
function calculateMinimumDealRORWA(){
    var spread=0.010000000;
    var targetMonths=[];
...
$('#btnCalculatePrice').on('click',function(){
    ...
    quotation.scenarios.push({spread:spread,sideBusinessValue:quotation._dealSideBusinessValue});
    ...
    targetCSR=quotation.seriesCSR[quotation.currentYearMonthIndex-1];
    $('#sideBusinessRangeSliderRange').val(dealRevenues*targetCSR);
    $('#sideBusinessRangeSliderRange').trigger('input');
    spread=calculateMinimumDealRORWA();
    quotation.scenarios.push({spread:spread,sideBusinessValue:quotation._dealSideBusinessValue});
